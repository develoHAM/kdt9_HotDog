...
const post_mypage = async (req, res) => {
	...
	User.findOne({
		...
		const { id, userid, pw, name, birth, phonenumber, address, dogname, RFIDcode, profile } = result;
		res.json({ result: true, id, userid, pw, name, birth, phonenumber, address, dogname, RFIDcode, profile });
	});
};
	...
	if (bearer === 'Bearer') {
		try {
			...
			if (returnValue) {
				const hash = await bcryptPassword(pw);
				User.update(
					{ userid, name, pw: hash, birth, phonenumber, address, dogname, RFIDcode },
...
const post_mypageProfile = async (req, res) => {
	try {
		console.log(req.file);
		const updateProfileObj = { profile: process.env.DEFAULT_PROFILE };
		if (req.file) {
			updateProfileObj.profile = req.file.transforms[0].location;
		}
		await User.update(updateProfileObj, {
			where: {
				userid: req.body.userid,
			},
		});

		res.status(200).json({ result: true, newProfile: updateProfileObj.profile });
	} catch (error) {
